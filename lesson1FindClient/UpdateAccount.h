...
static void PerformClientUpdate(ClsBankClient &client)
{
   int Choice = clsInputValidate::ReadIntNumberBetween("Enter Choice: ", 1, 8);
   switch (Choice)
   {
   case eFirstName:
       cout << "Enter First Name: ";
...
static void UpdateAccount(){
    ...
    while(count<3){
     ...
     if(!ClsBankClient::IsFound(accNum)){
        ...
        if(client.SaveToFile() == ClsBankClient::enSaveMode::SaveSucceded)
        {
